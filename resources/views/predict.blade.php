...
@auth
    @if(isset($dataPrediksi) && $dataPrediksi->count() > 0)
        <div class="bg-white rounded-xl shadow-md p-6">
            <div class="flex items-center justify-between mb-4 pb-3 border-b border-gray-200 flex-wrap gap-3">
                ...
                <span class="text-xs bg-green-100 text-green-700 px-3 py-1 rounded-full">
                    Total: {{ method_exists($dataPrediksi, 'total') ? $dataPrediksi->total() : $dataPrediksi->count() }} prediksi
                </span>
            </div>
                    ...
                    <tbody>
                        @foreach($dataPrediksi as $index => $item)
                            @php
                                ...
                                $nomorUrut = method_exists($dataPrediksi, 'currentPage') 
                                    ? ($dataPrediksi->currentPage() - 1) * $dataPrediksi->perPage() + $loop->iteration 
                                    : $loop->iteration;
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-3 py-2 border">{{ $nomorUrut }}</td>
            ...
            <div class="mt-4 p-3 bg-gray-50 rounded-lg">
                <p class="text-xs text-gray-600">
                    <strong>Total Prediksi:</strong> {{ method_exists($dataPrediksi, 'total') ? $dataPrediksi->total() : $dataPrediksi->count() }} kali |
        ...
        <!-- PAGINATION LINKS -->
        @if(method_exists($dataPrediksi, 'total') && $dataPrediksi->total() > $dataPrediksi->perPage())
        <div class="px-6 py-4 border-t border-gray-200 bg-white rounded-b-xl shadow-md">
            {{ $dataPrediksi->appends(request()->query())->links('pagination::tailwind') }}
        </div>
        @endif
    @endif
@endauth
