    ...
    <form action="{{ route('data_barang.update', $data_barang->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="nama" rows="2" 
                   ...
                   placeholder="Nama lengkap produk..." required>{{ old('nama', $data_barang->nama) }}</textarea>
            @error('nama')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="status_produk" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                <option value="Aktif" {{ old('status_produk', $data_barang->status_produk) == 'Normal' ? 'selected' : '' }}>Normal</option>
                <option value="Nonaktif" {{ old('status_produk', $data_barang->status_produk) == 'Diblokir' ? 'selected' : '' }}>Diblokir</option>
                <option value="Diskon" {{ old('status_produk', $data_barang->status_produk) == 'Diarsipkan' ? 'selected' : '' }}>Diarsipkan</option>
            </select>
        </div>
